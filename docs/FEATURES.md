...
| 11.2 | **Rate Limiting** | Per-IP token bucket. `RateLimitStrict` (10/min, burst 5) on `/auth/*` and the public form submit/confirm routes; `RateLimitModerate` (120/min, burst 30) on every authenticated route; `RateLimitGenerous` (240/min, burst 40) on the three public form GETs | -- | `rate_limit_test.go`: 13 tests | -- | -- | **partial** | No E2E or integration test. All three tiers are live, but the split is public-forms versus everything else: `RateLimitGenerous` is attached by `SetupFormPublicRoutes` to the form definition, `embed.js` and hosted-view GETs only, so authenticated reads are still not treated differently from authenticated writes. The inline comment on the `RateLimitModerate` line in `cmd/main.go` read `// 60 req/min` until 2026-09-18 and now states the real 120/min, burst 30. `DISABLE_RATE_LIMIT=true` bypasses only the strict tier |

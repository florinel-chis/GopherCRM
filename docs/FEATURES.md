...
| 11.5 | **Pagination Parameters** | `page`, `limit`, `offset`, `per_page` parsing | -- | `response_test.go`: TestParseOffsetLimit_Defaults, _Custom, _ExceedsMax, _NeverReturnsZeroLimit, _RejectsNegativeOffset, TestParsePaginationParams_Defaults, _Custom, _InvalidValues, _ExceedsMax | -- | -- | **covered** | `?limit=0` used to reach the pagination arithmetic and panic, turning every list endpoint into a 500 via one query parameter. `TestParseOffsetLimit_NeverReturnsZeroLimit` is the regression test |
